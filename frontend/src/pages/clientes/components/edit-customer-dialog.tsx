...
import { createCustomerSchema, type CreateCustomerSchema, type UpdateCustomerSchema } from "../schemas/customer.schema"
...
const EditCustomerDialog = ({customer}:{customer: Customer}) => {
  ...
  const {register, handleSubmit, formState: {errors}, reset, watch, setValue, control} = useForm({
    ...
    defaultValues: {
      ...
      data_nascimento:  customer.data_nascimento && formatDateAAAAMMDD(customer.data_nascimento),
      fone: customer.fone || '',
      endereco: customer.endereco || '',
      status:  customer.status,
      limiteCredito: Number(customer.limiteCredito)
    },
    mode: 'onChange'
  })
  ...
  const handleEditCustomer = async (data: UpdateCustomerSchema) => {
    ...
    try {
      ...
      setTimeout(()=> {
        ...
        toast.success('Edição de cliente', {
